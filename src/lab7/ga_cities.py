...
def game_fitness(cities, idx, elevation, size):
    ...
    """
    city_elevations = [elevation[city[0], city[1]] for city in cities]
    if any(i < 0 for i in city_elevations):
        return fitness

    center = np.mean(cities, axis=0)
    centerDistance = [np.linalg.norm(city - center) for city in cities]
    avgDistance = np.mean(centerDistance)
    centerDistancestd = np.std(center)
    fitness += 1 / (1 + avgDistance + centerDistancestd)
    

    for i in range(len(cities)):
        for j in range(i + 1, len(cities)):
            if np.all(cities[i] == cities[j]):
                return fitness
           
            distance = np.linalg.norm(cities[i] - cities[j])
            if distance < 5:  
                line = np.linspace(0, 1, int(distance))
                points = np.array([cities[i] * (1 - x) + cities[x] * x for x in line])
